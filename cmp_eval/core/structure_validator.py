...
Accepts both the canonical schema and common generator aliases:
  - study_id  OR  metadata.study_id
  - analysis_frequency  OR  analysis_schedule
  - thresholds{}  OR  flat moderate_threshold / high_threshold
  - weight "High"  OR  "2 (High), 1 (Moderate)"
  - QTL name  OR  qtl_name;  expectation_pct OR expectation;
    tolerance_limit_pct OR tolerance_limit
"""
...
# Aliases: if the canonical key is missing, accept these instead.
_TOP_LEVEL_ALIASES: dict[str, list[str]] = {
    "analysis_frequency": ["analysis_schedule"],
}
...
# Aliases for KRI fields the generator may use instead of the canonical name.
_KRI_FIELD_ALIASES: dict[str, list[str]] = {
    "thresholds": ["moderate_threshold", "high_threshold"],
}
...
_QTL_FIELD_ALIASES: dict[str, list[str]] = {
    "name": ["qtl_name"],
    "expectation_pct": ["expectation"],
    "tolerance_limit_pct": ["tolerance_limit"],
}
...
_WEIGHT_TOKEN_RE = re.compile(r'\b(High|Moderate|Low)\b', re.IGNORECASE)


def _normalise_weight(raw: str | None) -> str | None:
    """Extract the highest priority token from strings like '2 (High), 1 (Moderate)'."""
    if raw is None:
        return None
    if raw in VALID_WEIGHTS:
        return raw
    tokens = _WEIGHT_TOKEN_RE.findall(str(raw))
    if not tokens:
        return raw
    priority = {"high": 0, "moderate": 1, "low": 2}
    best = min(tokens, key=lambda t: priority.get(t.lower(), 99))
    return best.capitalize()


def _has_field_or_alias(obj: dict, canonical: str, aliases: dict[str, list[str]]) -> bool:
    """Return True if *obj* contains *canonical* or any of its aliases."""
    if canonical in obj:
        return True
...
def validate_structure(cmp_json: dict) -> StructureValidationResult:
    ...
    for key in REQUIRED_TOP_LEVEL_KEYS:
        if key == "study_id":
            # Accept study_id at top level OR inside metadata.study_id
            found = bool(cmp_json.get("study_id"))
            if not found:
        ...
        elif _has_field_or_alias(cmp_json, key, _TOP_LEVEL_ALIASES):
            result.ok()
        else:
            result.error("MISSING_SECTION", f"Required top-level key missing: '{key}'", f"$.{key}")

    # study_id non-null (check both locations)
    ...
    # ── Section 7: analysis_frequency / analysis_schedule ───────────────────
    af = cmp_json.get("analysis_frequency") or cmp_json.get("analysis_schedule")
...
def _validate_kri(kri: Any, path: str, ids_seen: set, result: StructureValidationResult):
    ...
    for field in KRI_REQUIRED_FIELDS:
        if _has_field_or_alias(kri, field, _KRI_FIELD_ALIASES):
            result.ok()
        else:
    ...
    # weight controlled vocabulary (normalise multi-token strings first)
    weight = _normalise_weight(kri.get("weight"))
    if weight is not None and weight not in VALID_WEIGHTS:
        result.error("KRI_INVALID_WEIGHT", f"weight '{kri.get('weight')}' not in {VALID_WEIGHTS}", f"{path}.weight")
    elif weight is not None:
        result.ok()

    # threshold_direction / relative_score_direction controlled vocabulary
    td = kri.get("threshold_direction") or kri.get("relative_score_direction")
    if td is not None and td.lower().strip() not in VALID_THRESHOLD_DIRECTIONS:
        result.error("KRI_INVALID_DIRECTION", f"threshold_direction '{td}' not in {VALID_THRESHOLD_DIRECTIONS}", f"{path}.threshold_direction")
    elif td is not None:
        result.ok()

    # thresholds structure (canonical nested dict or flat fields)
    thresholds = kri.get("thresholds", {})
    if isinstance(thresholds, dict) and thresholds:
        _validate_thresholds(thresholds, f"{path}.thresholds", result)

    # iqmp_risk_id format — accept iqmp_risk_id or asrp_risk_ids list
    iqmp = kri.get("iqmp_risk_id")
    if not iqmp:
        asrp_list = kri.get("asrp_risk_ids")
        if isinstance(asrp_list, list) and asrp_list:
            iqmp = ", ".join(str(x) for x in asrp_list)
...
def _validate_qtl(qtl: Any, path: str, result: StructureValidationResult):
    ...
    for field in QTL_REQUIRED_FIELDS:
        if _has_field_or_alias(qtl, field, _QTL_FIELD_ALIASES):
            result.ok()
        else:
            result.error("QTL_MISSING_FIELD", f"Required field '{field}' missing from QTL", f"{path}.{field}")

    # expectation_pct / expectation and tolerance_limit_pct / tolerance_limit
    for num_field in ["expectation_pct", "tolerance_limit_pct"]:
        val = qtl.get(num_field)
        if val is None:
            for alt in _QTL_FIELD_ALIASES.get(num_field, []):
                val = qtl.get(alt)
                if val is not None:
                    break
        if val is not None:
