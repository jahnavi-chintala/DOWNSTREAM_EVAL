...
def _resolve_pipd(study: str) -> tuple[Path, Path]:
    pj = Path(os.environ["PIPD_INPUT_JSON"]) if os.environ.get("PIPD_INPUT_JSON") else _PFIZER / "pipd_eval" / "data" / f"{study}_PIPD.json"
    gt = Path(os.environ["PIPD_GT_CSV"]) if os.environ.get("PIPD_GT_CSV") else _PFIZER / "pipd_eval" / "data" / "pipd_ground_truth.csv"
    return pj, gt
...
def main() -> int:
    ...
    if ok_pipd:
        pipd_root = _PFIZER / "pipd_eval"
        bench = pipd_root / "data" / "deviation_subcategories_clean.csv"
        if not bench.is_file():
            bench = pipd_root / "data" / "deviation_subcategories.csv"
        stem = f"pipd_eval_{study}"
        pipd_json_out = pipd_dir / f"{stem}.json"
        rc |= _run(
            "PIPD",
            pipd_root,
            [
