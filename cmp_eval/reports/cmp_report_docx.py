...
import reports.cmp_eval_docx_spec as _cmp_spec
...
_HEADER_SHADE = "003087"    # Pfizer blue — all table header rows
_STRUCTURE_SHADE = "FFF2CC"
_HIGH_PRIORITY_SHADE = "FFE6E6"
_MISS_COLOR = RGBColor(0xB9, 0x1C, 0x1C)   # fail / miss text
_GREEN_OK = RGBColor(0x1A, 0x73, 0x40)      # pass text
_WHITE = RGBColor(0xFF, 0xFF, 0xFF)          # header text on dark bg
...
def add_table(
    ...
    *,
    ...
    for ci, h in enumerate(headers):
        _set_cell_text(hdr_cells[ci], h, bold=True, size_pt=_BODY_PT, rgb=_WHITE)
...
def _build_cmp_metrics_spec_rows(report: Dict[str, Any]) -> List[List[str]]:
    """Emit the core CMP metric rows with stakeholder labels and sources."""
...
def write_cmp_eval_docx(report: Dict[str, Any], output_path: str) -> None:
    ...
    _set_cell_shading(sc_tbl.rows[0].cells[0], "EBF0FB")
    sp = sc_tbl.rows[0].cells[0].paragraphs[0]
    sp.text = ""
    _paragraph_style_run(sp, "How the Document Score is Calculated", size_pt=9, bold=True)
    sp2 = sc_tbl.rows[0].cells[0].add_paragraph()
    sc_parts = [
        "Global KRIs (35%) + Study-Specific KRIs (40%) + QTLs (20%) + Metadata (5%)",
        " = weighted section score",
        f"  x  structure factor ({dsb.get('structure_factor', 1.0):.3f})" if dsb.get("structure_factor") is not None else "",
        "  =  Document Score (0–100) | Pass threshold: 75  ·  Target: 80",
    ]
    r2 = sp2.add_run("".join(sc_parts))
    r2.italic = True
    r2.font.size = Pt(8)
    ...
    if srows:
        ...
        doc.add_paragraph()
        _cmp_spec.add_section_weighted_contribution_bar_table(doc, srows)
