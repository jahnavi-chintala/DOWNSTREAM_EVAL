...
# ── Internal modules ──────────────────────────────────────────────────────────
# Add current directory to path so imports work when called from project root
sys.path.insert(0, str(Path(__file__).parent))
load_dotenv(Path(__file__).resolve().parent / ".env")
from eval_scenario1 import run_scenario1_eval, classify_failures
from eval_scenario2 import run_scenario2_eval
from run_eval import detect_scenario, run_eval, save_results_json
from run_all_verify import run_all_verify
from pipd_composite_report import write_composite_report_files
...
def eval_scenario1_report(req: Scenario1ReportRequest) -> Dict:
    ...
    """
    from pipd_scenario1_report import write_scenario1_report_files
