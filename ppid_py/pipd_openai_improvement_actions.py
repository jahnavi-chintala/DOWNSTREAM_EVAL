...
def build_improvement_actions_context(s1: Dict[str, Any]) -> Dict[str, Any]:
...
def fetch_improvement_actions_narrative(s1: Dict[str, Any]) -> Optional[str]:
    """
    ...
    ctx = build_improvement_actions_context(s1)
    if (
    ...
    from pipd_composite_report import _openai_chat

    system = (
        "You write short, actionable remediation summaries for developers of a PIPD "
        "(protocol digitization) generator. "
