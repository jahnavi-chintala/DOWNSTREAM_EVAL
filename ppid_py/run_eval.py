...
# ── Internal modules ──────────────────────────────────────────────────────────
from eval_scenario1 import run_scenario1_eval, classify_failures, filter_verify_split_rows
from eval_scenario2 import run_scenario2_eval
from pipd_semantic_review import (
...
def save_hallucination_report(
    ...
    """
    Write the hallucination detection report as a per-study JSON file.

    Format mirrors the spec:
      { study_id, hallucinations_found, flagged_subcategories, pass }
    ...
    print(f"[INFO] Hallucination report -> {report_path}")
...
def print_summary(results: Dict) -> None:
    ...
    if scenario == 1:
        ...
        rows = [
            ...
            ("M4 Hallucinations",        str(m['m4_hallucination_detection']['hallucinations_found']), m['m4_hallucination_detection']['pass']),
        ]
...
def _emit_scenario1_yaml_word(
    ...
    *,
    ...
    if not write_yaml and not write_word:
        return
    try:
        from pipd_scenario1_report import (
