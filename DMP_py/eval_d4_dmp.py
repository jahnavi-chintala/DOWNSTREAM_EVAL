...
from dmp_data import (
    infer_study_id,
    load_dmp_gt_record,
    load_dmp_json,
    load_sds_rows,
)
from report_builder import build_report, load_eval_config
from semantic_matcher import SemanticMatcher
from scorer_s5 import score_s5
from scorer_s6 import score_s6
from scorer_s8 import score_s8
from scorer_s11 import score_s11
...
def main() -> int:
    ...
    ap.add_argument(
        "--config",
        type=str,
        default=str(_PKG / "eval_config" / "dmp_eval_config.yaml"),
    )
    ap.add_argument(
        "--dmp-gt",
        type=str,
        default=str(_PKG / "data" / "dmp_ground_truth_clean.json"),
    )
    ap.add_argument(
        "--sds-gt",
        type=str,
        default=str(_PKG / "data" / "sds_non_crf_ground_truth_clean.csv"),
    )
