...
def validate_iqmp_risk_id(generated_id: any, gt_id: any) -> float:
    """
    When GT IQMP is blank, do not treat a valid SR- ID as a conflict — full credit
    if format is valid. When GT specifies codes, require match.
    """
    blank = gt_iqmp_is_blank(gt_id)

    def _valid_sr(g: any) -> bool:
        if not g:
            return False
        gs = str(g).strip()
        if not gs or gs.lower() in ("null", "none", ""):
            return False
        return bool(re.search(r"SR-\d{4,}", gs, re.IGNORECASE))

    if not generated_id:
        return 1.0 if blank else 0.0
    gen_str = str(generated_id).strip()
    if not gen_str or gen_str.lower() in ("null", "none", ""):
        return 1.0 if blank else 0.0

    if blank:
        return 1.0 if _valid_sr(generated_id) else 0.0

    if not _valid_sr(generated_id):
        return 0.0

    gt_str = str(gt_id).strip()
    if not gt_str or gt_str.lower() in ("nan", "none", "null", ""):
        return 1.0 if _valid_sr(generated_id) else 0.0

    gt_sr_codes = re.findall(r"SR-\d{4,}", gt_str, re.IGNORECASE)
    if any(gen_str.upper() == c.upper() for c in gt_sr_codes):
        return 1.0
    if gt_sr_codes:
        return 0.0
    return 0.5
...
def score_weight(generated: any, ground_truth: any, vocab: list) -> float:
    """Exact match on controlled vocabulary."""
    if not generated or not ground_truth:
        return 0.0
