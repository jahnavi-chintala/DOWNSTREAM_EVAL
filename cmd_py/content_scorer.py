...
class KRIAttributeScorer:
    ...
    def score_kri_pair(self, generated_kri: dict, gt_row: pd.Series) -> dict:
        ...
        iqmp_score = validate_iqmp_risk_id(
            generated_kri.get("iqmp_risk_id"),
            gt_iqmp,
        )
        ...
        results["iqmp_risk_id"] = {
            "score": iqmp_score,
            "generated": generated_kri.get("iqmp_risk_id"),
            "ground_truth": str(gt_row.get("iqmp_risk_id", "")),
            "weight": w,
            "is_hallucination": (
                not gt_blank
                and iqmp_score == 0.0
                and generated_kri.get("iqmp_risk_id") is not None
            ),
        }
        total_weight += w
...
class SectionScorer:
    ...
    def score_ss_kris(self, study_id: str, cmp_json: dict) -> dict:
        ...
        # Also check for sister KRIs from ground truth
        gt_sister = self.gt.get_active_kris(study_id, "sister")
        # Merge ss + sister for scoring denominator
        gt_combined = pd.concat([gt_ss, gt_sister], ignore_index=True) if not gt_sister.empty else gt_ss
        return self._score_kri_section(gen_ss, gt_combined, "study_specific")
    ...
    def score_metadata(self, study_id: str, cmp_json: dict) -> dict:
        """Score metadata section (analysis_frequency etc.)."""
        af = cmp_json.get("analysis_frequency", {})
        has_frequency = bool(af and (af.get("enrollment_phase") or af.get("first_kri_trigger")))
            ...
            if best_s < 0.35:
                ...
                for candidate_gt in gt_qtl_names:
                    ...
                    # Key fragment: "percentage of subjects" vs "% of subjects …"
                        ...
                        jac = jaccard_similarity(gen_name, candidate_gt)
                        if jac >= 0.35:
...
def calculate_m4_hallucinations(section_results: dict, cmp_json: dict, config: dict) -> dict:
    ...
    M4: Hallucinations.
    Counts:
      1) Matched KRIs where IQMP mapping is flagged hallucinated by attribute scorer
    ...
    return {
        ...
        "detail": f"{count} hallucinations detected (invalid IQMP mappings + extra KRIs/QTLs)",
        "note": "M4 includes existence-level hallucinations (extra generated items with no GT match).",
    }
