...
_DEFAULT_EVAL_CONFIG = _SCRIPT_DIR / "eval_config" / "risk_profile_eval_config.yaml"

# ── Third-party ───────────────────────────────────────────────────────────────
import pandas as pd

# ── Internal ──────────────────────────────────────────────────────────────────
from eval_scenario1 import run_scenario1_eval, classify_failures, VERIFY_STUDIES
from eval_scenario2 import run_scenario2_eval
from risk_profile_eval_metadata import build_eval_metadata
...
def save_hallucination_report(
    ...
    *,
    ...
    """
    Write the hallucination report to {study_id}_hallucination_report.json.

    Written for both scenarios:
      - Scenario 1: from M4 (hallucination_detection metric)
      - Scenario 2: from S1 (hallucination_signals signal)

    An empty flagged_fields list confirms the check ran and found nothing.
    A non-zero count is a CRITICAL finding requiring immediate generator fix.
...
def print_summary(results: Dict) -> None:
    ...
    if scenario == 1:
        ...
        m4p = m4.get("provenance_defect_count", m4.get("hallucinations_found", 0))
        m4s = m4.get("semantic_hallucination_count", 0)
        print(f"  M4 Provenance defects: {m4p}  ({badge(m4.get('passed'))})  [semantic unmatched: {m4s}]")
...
def _write_eval_word_report(report: Dict, output_path: Path) -> str:
    """Write eval Word report; on PermissionError use a timestamped filename. Returns path written."""
    from datetime import datetime as _dt

    from risk_profile_eval_report_docx import write_risk_profile_eval_docx
...
def run_eval(
    ...
    *,
    ...
    if write_yaml:
        try:
            from risk_profile_eval_report_yaml import write_risk_profile_eval_report_yaml
    ...
    if write_word:
        try:
            ...
            word_path = _write_eval_word_report(results, wp)
