...
Computes 4 metrics per the D1 Risk Profile Eval Framework Design:
  ...
  M1 – Risk Name Selection Recall   (CRITICAL  – target >= 85%; exact risk_name within same risk_domain when both sides declare domain)
  M2 – RPN Tier Accuracy            (CRITICAL  – target >= 90%; ±1 tier on M1-matched risks)
  M3 – Critical Factor Match        (HIGH      – target >= 80%; F1 on factor names)
  M4 – Traceability Flags           (CRITICAL  – target = 0 unresolved provenance fields)

Also emits ``hierarchy_verification`` (domain → risk → control_count vs generated controls length; advisory).
...
from core.risk_generator_risks import (
    domains_compatible,
    fingerprint_risk_name_for_m1,
    get_all_risk_dicts,
...
_LLM_OOS_CACHE: Dict[str, Dict[str, Any]] = {}
...
# Fields that must have provenance – used by M4 traceability checks
...
def find_near_misses(
    ...
    ``fingerprint_risk_name_for_m1`` (cosmetic chars removed); not counted as matches for M1.
    """
    near_misses = []
    for gt in unmatched_gt:
        tn = fingerprint_risk_name_for_m1(str(gt["risk_name"]).strip())
        best_dist = NEAR_MISS_THRESHOLD + 1
        best_gen = None
        for gr in unmatched_gen:
            gn = fingerprint_risk_name_for_m1(str(gr["risk_name"]).strip())
            d = lev(gn, tn)
...
def compute_risk_name_recall(
    ...
    """
    M1 – Risk name recall. Verbatim = same ``fingerprint_risk_name_for_m1`` (after
    slash/whitespace/ePRO normalisation, then ignore spaces, commas, periods, hyphens, underscores only).
    One-to-one greedy matching. **Domain-aware:** when both GT and generated rows carry a
    non-empty ``risk_domain`` (normalised), names must match **within the same domain**
    (Study Risks, Vendor Risks, Study Site Risks, Other Domain Risks) so e.g. the same
    risk title under Vendor vs Study does not cross-pair. If either side omits domain,
    behaviour falls back to name-only pairing. Denominator = **every** non-blank GT risk
    row for the study (dynamic per protocol — no fixed cap).
    ...
    for gi in eval_indices:
        ...
        for gj, gr in enumerate(gen_rows):
            # M1 verbatim: same fingerprint (cosmetic-only diffs: spaces, comma, . - _)
            if fingerprint_risk_name_for_m1(gt["risk_name"]) != fingerprint_risk_name_for_m1(
                gr["risk_name"]
            ):
                continue
            if not domains_compatible(gt["domain_norm"], gr["domain_norm"]):
                continue
    ...
    for gi, gj in pair_candidates:
        ...
        matched_pairs.append({
            ...
            "generated_list_key": gr["json_key"],
            "generated_list_index": int(gr["idx"]),
...
def _risk_dict_at(generator_json: Dict[str, Any], list_key: str, idx: int) -> Optional[Dict[str, Any]]:
    block = generator_json.get(list_key)
    if not isinstance(block, list) or idx < 0 or idx >= len(block):
        return None
    item = block[idx]
    return item if isinstance(item, dict) else None


def _parse_gt_control_count(raw: Any) -> Optional[int]:
    if raw is None:
        return None
...
def _gt_csv_row_for_pair(truth_df: pd.DataFrame, pair: Dict[str, Any]) -> Optional[pd.Series]:
    rid = str(pair.get("gt_risk_id") or "").strip()
    if rid and rid.lower() != "nan" and "risk_id" in truth_df.columns:
        m = truth_df[truth_df["risk_id"].astype(str).str.strip() == rid]
        if len(m) == 1:
            return m.iloc[0]
    fp = fingerprint_risk_name_for_m1(str(pair.get("gt_risk_name") or ""))
    gd = normalize_domain_label(str(pair.get("gt_domain") or ""))
    for _, row in truth_df.iterrows():
        if fingerprint_risk_name_for_m1(str(row.get("risk_name", "") or "")) != fp:
            continue
        rdom = normalize_domain_label(str(row.get("risk_domain", "") or ""))
        if gd and rdom and gd != rdom:
            continue
        return row
    return None


def compute_hierarchy_verification(
    generator_json: Dict[str, Any],
    risk_truth_df: pd.DataFrame,
    matched_pairs: List[Dict[str, Any]],
) -> Dict[str, Any]:
    """
    Domain → risk → controls: informational alignment of benchmark ``control_count``
    with each matched risk's ``controls`` list length. Does **not** gate GO/NO-GO.
    """
    dom_gt: Dict[str, int] = {}
    dom_gen: Dict[str, int] = {}
    dom_matched: Dict[str, int] = {}

    if not risk_truth_df.empty and "risk_domain" in risk_truth_df.columns:
        for _, row in risk_truth_df.iterrows():
            rn = str(row.get("risk_name", "") or "").strip()
            if not rn:
                continue
            d = normalize_domain_label(str(row.get("risk_domain", "") or ""))
            dom_gt[d or "(no domain)"] = dom_gt.get(d or "(no domain)", 0) + 1

    for json_key, _j, risk in iter_risk_dicts_with_keys(generator_json):
        rn = str(risk.get("risk_name", "") or "").strip()
        if not rn:
            continue
        d = normalize_domain_label(infer_generated_domain(json_key, risk))
        dom_gen[d or "(no domain)"] = dom_gen.get(d or "(no domain)", 0) + 1

    for p in matched_pairs:
        d = normalize_domain_label(str(p.get("gt_domain") or p.get("generated_domain") or ""))
        dom_matched[d or "(no domain)"] = dom_matched.get(d or "(no domain)", 0) + 1

    per_risk: List[Dict[str, Any]] = []
    rates: List[float] = []

    for p in matched_pairs:
        lk = p.get("generated_list_key") or p.get("generated_json_key")
        idx = p.get("generated_list_index")
        gen_risk = None
        if isinstance(lk, str) and isinstance(idx, int):
            gen_risk = _risk_dict_at(generator_json, lk, idx)
        gen_cc = len(gen_risk.get("controls") or []) if gen_risk else None

        gt_row = _gt_csv_row_for_pair(risk_truth_df, p) if not risk_truth_df.empty else None
        gt_cc = _parse_gt_control_count(gt_row.get("control_count")) if gt_row is not None else None

        match: Optional[bool] = None
        if gt_cc is not None and gen_cc is not None:
            match = gt_cc == gen_cc
            rates.append(1.0 if match else 0.0)

        per_risk.append({
            "gt_risk_id": p.get("gt_risk_id"),
            "gt_risk_name": p.get("gt_risk_name"),
            "gt_domain": p.get("gt_domain"),
            "gt_control_count": gt_cc,
            "generated_control_count": gen_cc,
            "control_count_match": match,
        })

    rate = round(sum(rates) / len(rates), 4) if rates else None

    return {
        "note": (
            "IRMS-style layout: domains (Study / Vendor / Study Site / Other) contain risks; "
            "each risk lists controls. M1 pairs risks within domain when domains are present; "
            "this block compares benchmark control_count to len(controls) per matched pair."
        ),
        "by_domain": {
            "ground_truth_risk_rows": dom_gt,
            "generated_risk_rows": dom_gen,
            "m1_matched_pairs": dom_matched,
        },
        "per_risk_control_alignment": per_risk,
        "control_count_match_rate": rate,
        "pairs_with_gt_control_count": len(rates),
    }
...
def compute_critical_factor_match(
    ...
    """
    M3 – Critical factor name matching. The official score is factor-name F1
    so both missing GT factors and extra generated factors affect the score.
    Studies in ``FACTOR_NAME_ONLY_STUDIES`` skip content checks when GT marks
    data/process blank.
    ...
    # M3 should reflect both missed and extra critical factors. Recall alone
    # can show 100% even when the generator emits non-GT factors, so use F1 as
    # the official score while still exposing precision/recall separately.
    score = round(f1, 4)
...
def compute_hallucination_detection(
    ...
    *,
    ...
    """
    Compute M4 – Traceability Flags.

    Flags two classes of unresolved provenance issue:
    ...
    2. **Trace flags** — every reference that *is* present must actually
       resolve to a node in the uploaded USDM JSON (by id, or by
       ``(instanceType, name)`` match). References that don't resolve are
       reported as traceability flags, not confirmed semantic hallucinations.
       See ``risk_usdm_tracing.trace_reference``. References that resolve to
       ``type_only`` (declared instanceType present in protocol with no asserted
       id / placeholder id) are acceptable and are not counted toward M4 — same
       rule as PIPD protocol traceability.
    ...
    # generator must output zero risks. Any generated risk is a benchmark extra.
    ...
    flagged, oos_rows, llm_meta = _exclude_out_of_scope_trace_flags(flagged)
    provenance_count = len(flagged)
    return {
        "provenance_defect_count": provenance_count,
        "traceability_flag_count": provenance_count,
        ...
        "out_of_scope_not_penalized": oos_rows,
        "llm_oos_meta": llm_meta,
        ...
        "note": (
            "M4 counts unresolved provenance/schema traceability flags per risk field, not "
            "confirmed semantic hallucinations. The legacy hallucinations_found field is kept "
            "for compatibility and equals traceability_flag_count. Use "
            "semantic_hallucination_count (from M1) for generated risks with no GT match."
        ),
    }


# ─────────────────────────────────────────────────────────────────────────────
# LLM out-of-scope filter (optional, soft-fail)
# ─────────────────────────────────────────────────────────────────────────────

def _openai_chat_json(messages: List[Dict[str, str]], *, max_tokens: int = 700) -> Tuple[Optional[str], Optional[str]]:
    """Minimal OpenAI chat caller; returns (text, error)."""
    key = os.getenv("OPENAI_API_KEY", "").strip()
    if not key:
        return None, "OPENAI_API_KEY missing"
    model = os.getenv("OPENAI_MODEL", "").strip()
    if not model:
        return None, "OPENAI_MODEL missing"
...
def _llm_classify_oos(row: Dict[str, Any]) -> Dict[str, Any]:
    """Classify whether one trace defect is not predictable from protocol text."""
    ...
    if cache_key in _LLM_OOS_CACHE:
        return _LLM_OOS_CACHE[cache_key]

    # Only trace defects are candidates; keep others penalized.
    if not row.get("trace_status"):
        out = {"out_of_scope": False, "reason": "Schema/existence defect; in-scope"}
        _LLM_OOS_CACHE[cache_key] = out
        return out

    default = {"out_of_scope": False, "reason": "LLM unavailable; kept penalized"}
    system = (
        "Decide if a Risk Profile trace defect is out-of-scope for protocol-text prediction. "
        "Return JSON only: {\"out_of_scope\": boolean, \"reason\": string}. "
        "Use out_of_scope=true only for operational/runtime/system issues not inferable from protocol "
        "(internet outage, site process drift, vendor connectivity, IT execution errors). "
        "USDM mapping, missing ids, wrong entity/signal, unresolved references are usually in-scope=false."
    )
    user = json.dumps(
        {
            "field_path": row.get("field_path"),
    ...
    text, err = _openai_chat_json(
        [{"role": "system", "content": system}, {"role": "user", "content": user}],
        max_tokens=int(os.getenv("RISK_OOS_MAX_TOKENS", "300")),
    )
    if err:
        _LLM_OOS_CACHE[cache_key] = default
        return default
    ...
    if not obj:
        ...
        return default
    out = {
        "out_of_scope": bool(obj.get("out_of_scope")),
        "reason": str(obj.get("reason") or "").strip()[:500] or "LLM classified",
    }
    _LLM_OOS_CACHE[cache_key] = out
    return out


def _exclude_out_of_scope_trace_flags(flagged: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], Dict[str, Any]]:
    """Exclude LLM-classified out-of-scope trace defects from M4 penalties."""
    enabled = os.getenv("RISK_PROFILE_LLM_OOS", "1").strip().lower() in ("1", "true", "yes", "on")
    if not enabled:
        return flagged, [], {"enabled": False, "excluded_count": 0}
    ...
    return kept, excluded, {"enabled": True, "llm_checked": checked, "excluded_count": len(excluded)}
...
def run_scenario1_eval(
    ...
    *,
    ...
    and adds timestamp + metadata. Also attaches ``hierarchy_verification`` (domain →
    risk → control count alignment per M1 pairs; informational, does not change verdict).
    This is the primary entry point called by run_eval.py and api.py.
    ...
    hierarchy = compute_hierarchy_verification(
        generator_json,
        risk_truth_df,
        m1.get("matched_pairs", []) or [],
    )

    # ── M2 (only on M1 matched pairs; domain-aware when GT lists risk_domain) ───
    ...
    return {
        ...
        "hierarchy_verification": hierarchy,
        "metadata_validation": metadata_validation,
        "hallucination_report": {
            "study_id": study_id,
            "provenance_defect_count": m4.get("provenance_defect_count", m4.get("hallucinations_found")),
            "traceability_flag_count": m4.get("traceability_flag_count", m4.get("hallucinations_found")),
            ...
            "traceability_flags": m4.get("traceability_flags", m4["flagged_fields"]),
            "out_of_scope_not_penalized": m4.get("out_of_scope_not_penalized", []),
            "pass": m4["passed"],
            "note": m4.get("note"),
        },
        "gt_risk_exists": gt_risk_exists,
...
def classify_failures(results: Dict) -> List[Dict]:
    ...
    if m3 and not m3.get("skipped") and not m3.get("passed"):
        ...
        if m3.get("benchmark_expects_zero_factors") and int(m3.get("ground_truth_total") or 0) == 0:
            ...
            failures.append({
                ...
                "target": TARGETS["m3_critical_factor_match"],
                "detail": (
                    f"Name F1={m3.get('score')} (recall={m3.get('recall')}, precision={m3.get('precision')}), "
                    f"matched names={m3.get('matched_factors')}/{m3.get('ground_truth_total')}, "
                    f"missing={m3.get('missing_names')}, extras={m3.get('extra_names')}. "
                    f"content notes={cv}."
                ),
    ...
    if m4 and not m4.get("passed"):
        ...
        if existence_mismatch or cf_existence_mismatch:
            ...
            m4_detail = (
                f"Traceability/provenance flags: {prov} (target 0). "
                f"Confirmed semantic hallucinations from M1 unmatched generated risks: {sem}. "
                f"Flagged fields ({len(flagged)}): {flagged}"
            )
            if oos:
                m4_detail += (
                    f" Out-of-scope (not penalized, non-predictable from protocol text): "
                    f"{len(oos)} row(s)."
                )
            m4_root = "Generated references are missing explicit provenance or do not resolve under the current USDM matching rules"
            m4_fix = "Populate concrete USDM ids where available; normalize entity labels to USDM instanceType names; review eval alias/matching rules before treating these as hallucinations"
        failures.append({
            "study_id": study_id,
...
if __name__ == "__main__":
    ...
    print(f"M4 Traceability flags:  {m4p}  (target 0)  "
