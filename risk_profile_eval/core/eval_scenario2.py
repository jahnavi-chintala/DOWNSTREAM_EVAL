...
def check_risk_count_sanity(generator_json: Dict) -> Dict:
    ...
    """
    study_risks  = get_all_risks(generator_json)         # gen["risks"]
    vendor_risks = generator_json.get("vendor_risks", [])
    site_risks   = generator_json.get("study_site_risks", [])
    study_count  = len(study_risks)
    total_count  = study_count + len(vendor_risks) + len(site_risks)

    # Study-risk count gate (3–6 expected for the main Study domain)
    if study_count < RISK_COUNT_FLOOR or study_count > RISK_COUNT_CEILING:
        status = "WARN"   # WARN not FAIL — other domains compensate
        description = (
            f"Study-domain risks: {study_count} "
            f"(expected {RISK_COUNT_FLOOR}–{RISK_COUNT_CEILING}).  "
            f"Total across all domains: {total_count} "
            f"(Vendor: {len(vendor_risks)}, Site: {len(site_risks)}).  "
            f"{'Generator ceiling violated for study domain.' if study_count > RISK_COUNT_CEILING else 'Generator produced fewer study-domain risks than expected.'}"
        )
    else:
        status = "PASS"
        description = (
            f"Study-domain risks: {study_count} — within expected {RISK_COUNT_FLOOR}–{RISK_COUNT_CEILING}.  "
            f"Total across all domains: {total_count} "
            f"(Vendor: {len(vendor_risks)}, Site: {len(site_risks)})."
        )
    ...
    return {
        ...
        "status": status,
        "study_risk_count": study_count,
        "vendor_risk_count": len(vendor_risks),
        "site_risk_count": len(site_risks),
        "total_risk_count": total_count,
        "floor": RISK_COUNT_FLOOR,
        "ceiling": RISK_COUNT_CEILING,
        "additional_domain_warnings": [],
...
def check_placeholder_ids(generator_json: Dict) -> Dict:
    ...
    Returns:
        Signal dict: {signal_id, name, status (PASS|FAIL), violations}
    """
    # Placeholder patterns per domain
    _DOMAIN_PATTERNS = {
        "study":  (re.compile(r"^SR-PLACEHOLDER-\d{3}$"),  "SR-PLACEHOLDER-XXX"),
        "vendor": (re.compile(r"^VR-PLACEHOLDER-\d{3}$"),  "VR-PLACEHOLDER-XXX"),
        "site":   (re.compile(r"^SSR-PLACEHOLDER-\d{3}$"), "SSR-PLACEHOLDER-XXX"),
    }
    SCT_PATTERN = re.compile(r"^SCT-PLACEHOLDER-\d{3}$")
    violations = []

    domain_map = [
        ("study",  get_all_risks(generator_json)),
        ("vendor", generator_json.get("vendor_risks", [])),
        ("site",   generator_json.get("study_site_risks", [])),
    ]
    total_checked = 0
    for domain_key, domain_risks in domain_map:
        pat, expected_fmt = _DOMAIN_PATTERNS[domain_key]
        for i, risk in enumerate(domain_risks):
            total_checked += 1
            risk_name = risk.get("risk_name", f"{domain_key}Risk[{i}]")
            risk_id   = risk.get("risk_id", "")
            if not pat.match(str(risk_id)):
                violations.append({
                    "risk_name": risk_name,
                    "domain":    domain_key,
                    "field":     "risk_id",
                    "value":     risk_id,
                    "reason":    f"Expected {expected_fmt}, got '{risk_id}' — may be a real IRMS ID",
                })
            for j, control in enumerate(risk.get("controls", [])):
                ...
                if not SCT_PATTERN.match(str(control_id)):
                    violations.append({
                        ...
                        "value":     control_id,
                        "reason":    f"Expected SCT-PLACEHOLDER-XXX, got '{control_id}'",
                    })
    ...
    return {
        ...
        "status": "FAIL" if violations else "PASS",
        "total_risks": total_checked,
        "violations": violations,
        "description": (
            f"{len(violations)} non-placeholder ID(s) found across all domains "
            "(Study/Vendor/Site) — generator may be using real IRMS IDs from training data."
            if violations else
            "All risk_ids and control_ids use correct placeholder format."
        ),
    }
