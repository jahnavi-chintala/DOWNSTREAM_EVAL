...
# Reference-style eval report (stakeholder template): blue header band, green pass text
_HEADER_BLUE = "4472C4"
_RGB_OK = RGBColor(0x00, 0x66, 0x00)
_RGB_FAIL = RGBColor(0xC0, 0x00, 0x00)
...
def _risk_by_name(gen: Optional[Dict[str, Any]], name: str) -> Optional[Dict[str, Any]]:
    if not gen:
        return None
    from risk_generator_risks import get_all_risk_dicts
...
def _rpn_tier_pass_for_row(
    ...
    if tr is None or (isinstance(tr, float) and str(tr) == "nan"):
        return bool(m2row and m2row.get("passed"))
    try:
        from eval_scenario1 import tier_distance
...
def _format_rpn_tier_display(rpn_val: Any) -> str:
    try:
        ...
        return str(rpn_val)
    from eval_scenario1 import rpn_to_tier
...
def add_risk_block(
    ...
    doc.add_heading("Risk match information", level=3)
    from risk_generator_risks import normalize_risk_name_for_match

    gn = normalize_risk_name_for_match(str(gen_risk.get("risk_name", "") or ""))
    tn = normalize_risk_name_for_match(str(risk_name or ""))
    names_norm_match = bool(gn and tn and gn == tn)
    lev = 0 if m1_matched else "—"
    if m1_matched:
        near_txt = f"Verbatim ✓ | Levenshtein: {lev}"
    elif names_norm_match:
        near_txt = "Normalized name match ✓ (not in M1 domain-matched set)"
    else:
    ...
    gen_domain = str(gen_risk.get("risk_domain") or "—")
    gt_domain_val = (
    ...
    gen_domain_s = str(gen_risk.get("risk_domain") or "").strip()
    gt_domain_val2 = (
        str(gt_row["risk_domain"]).strip()
        if gt_row is not None and pd_safe(gt_row, "risk_domain")
        else ""
    ...
    if ctrl_gt_first and ctrl_gt_first != "—":
        ctrl_ok = pri_ctrl.strip().lower() == ctrl_gt_first.lower()
        ctrl_score = "1.00" if ctrl_ok else "0.00"
        ctrl_match = "exact" if ctrl_ok else "miss"
    elif pri_ctrl:
        ctrl_score = "—"
        ctrl_match = "N/A"
        ctrl_gt_first = "—"
...
def _inventory_generated_risks(gen: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    from risk_generator_risks import infer_generated_domain, iter_risk_dicts_with_keys
...
def _write_scenario1(doc: Document, report: Dict[str, Any]) -> None:
    ...
    p.text = ""
    _para_line_spacing(p)
    _run(p, pass_txt, bold=True, size_pt=_HEADER_PT, rgb=RGBColor(0x00, 0x66, 0x00) if doc_pass else RGBColor(0xC0, 0x00, 0x00))
    ...
    if gt_df is not None and not gt_df.empty:
        for _, grow in gt_df.iterrows():
            ...
            rs = _per_risk_display_score(rname, matched, m2r, genr, grow)
    ...
    # ── 3. Risk detail ─────────────────────────────────────────────────────
    add_section_title(doc, "3. Risk Detail")
    if gt_df is not None and not gt_df.empty:
        for i, (_, grow) in enumerate(gt_df.iterrows(), start=1):
            rname = str(grow["risk_name"])
            pair = _pair_for_gt_row(matched_pairs, grow)
            genr = _gen_risk_from_pair(gen, pair) if pair else _risk_by_name(gen, rname)
            m2r = _m2_row_for_gt(metrics, grow)
            matched = pair is not None
            add_risk_block(doc, i, rname, genr, grow, m2r, matched)
    else:
        inv = inv_when_no_gt
        if inv:
            p = doc.add_paragraph()
    ...
    # ── 4. Critical factors ─────────────────────────────────────────────────
    add_section_title(doc, "4. Critical Factors")
    ...
    add_section_title(doc, "5. Hallucination Check")
    ...
    # ── 6. Near misses ──────────────────────────────────────────────────────
    add_section_title(doc, "6. Near Misses")
