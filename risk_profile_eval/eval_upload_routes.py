...
from eval_scenario1 import classify_failures
...
_DEFAULT_RISKS_CANDIDATES = (
    _PKG / "data" / "risk_profile_ground_truth.csv",
    _PKG.parent / "risk_profile_ground_truth.csv",
)
_DEFAULT_FACTORS_CANDIDATES = (
    _PKG / "data" / "critical_factors_ground_truth.csv",
    _PKG.parent / "critical_factors_ground_truth.csv",
)
    ...
    if sc == 1:
        ...
        prev["metric_rows"] = [
            ...
            },
            {
                ...
                "detail": str(m4.get("provenance_defect_count", m4.get("hallucinations_found", "—"))),
                "pass": bool(m4.get("passed")),
                "tooltip": (
                    "USDM-trace defects: generator references (usdm_id or entity+signal) that do not resolve "
                    "to any node in the protocol USDM. See the tracing panel below for per-row detail."
                ),
            },
        ]
        notes: list[str] = []
        ...
        prev["counts"] = {"near_misses": len(m1.get("near_misses") or [])}
        ...
        for sid, blk in signals.items():
            st = blk.get("status", "")
            rows.append({"metric": str(sid), "detail": st, "pass": st == "PASS"})
        prev["metric_rows"] = rows[:12]
        ...
        prev["failure_notes"] = notes
        prev["counts"] = {"human_review_items": int(result.get("review_list_count") or 0)}
