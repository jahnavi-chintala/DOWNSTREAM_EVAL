...
def _abbreviation_match(a: str, b: str) -> bool:
    """Return True when one string is the abbreviation/expanded form of the other.

    Handles patterns like:
      "DMW" vs "Data Management Workbench (DMW)"
      "CORD" vs "Clinical Oversight Review Dashboard (CORD)"
      "Smart Data Query" vs "Smart Data Query (SDQ)"
    """
    import re
    short, long = (a, b) if len(a) <= len(b) else (b, a)
    short_l = short.strip().lower()
    long_l = long.strip().lower()
    if not short_l or not long_l:
        return False
    # Check if short appears inside parentheses in long
    paren_match = re.search(r"\(([^)]+)\)", long_l)
    if paren_match and paren_match.group(1).strip() == short_l:
        return True
    # Check if long appears inside parentheses in short (reversed)
    paren_match = re.search(r"\(([^)]+)\)", short_l)
    if paren_match and paren_match.group(1).strip() == long_l:
        return True
    # Check if removing the parenthetical from the longer string matches the shorter
    long_stripped = re.sub(r"\s*\([^)]*\)\s*$", "", long_l).strip()
    if long_stripped and long_stripped == short_l:
        return True
    short_stripped = re.sub(r"\s*\([^)]*\)\s*$", "", short_l).strip()
    if short_stripped and short_stripped == long_l:
        return True
    return False
...
def verbatim_semantic_attribute_score(
    ...
    # Abbreviation-in-parentheses: "DMW" matches "Data Management Workbench (DMW)"
    if _abbreviation_match(gt_s, gen_s):
        return 1.0, "verbatim", detail
