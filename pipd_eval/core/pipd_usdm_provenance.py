...
from core.eval_scenario1 import NULL_PLACEHOLDERS
...
def broad_usdm_search(
    pipd_text: str,
    all_nodes_by_id: Dict[str, Dict[str, Any]],
    top_k: int = 5,
    min_ratio: float = 0.18,
) -> List[Tuple[float, Dict[str, Any]]]:
    """Search ALL USDM nodes (regardless of type) for the best match to pipd_text.

    Returns top_k results sorted by descending lex score, filtered to min_ratio.
    Only nodes that have a real ``id`` field are included so we can always report
    a concrete USDM entity ID.

    This is the **evaluator-driven** lookup — it does not rely on the generator's
    ``usdm_entity`` class assignment at all.
    """
    pt = pipd_text.lower().strip()[:800]
    ...
    for node in all_nodes_by_id.values():
        ...
        if not blob.strip():
            continue
        r = lev_ratio(pt, blob[:4000])
        if r >= min_ratio:
            scored.append((r, node))
...
def resolve_usdm_source_for_subcategory(
    ...
    """Resolve the best USDM source for a PIPD subcategory.

    Strategy (in order):
    1. **id_lookup** – generator provided a non-null ``usdm_entity_id`` → look it up directly.
    2. **broad_eval** – evaluator-driven broad search across ALL USDM nodes (ignores the
       generator's class assignment). Returns a real entity ID and type from the USDM.
    3. **type_match** – fall back to type-restricted search using generator's ``usdm_entity``
       class if broad search found nothing useful.

    Returns
    -------
    (display_line, method, eval_entity_id, eval_entity_type)
        display_line    : human-readable provenance string (or None)
        method          : id_lookup | broad_eval | type_match | ai_assisted | none
        eval_entity_id  : entity ID found by evaluator (may be None)
        eval_entity_type: instanceType of the found node (may be None)
    """
    ...
    # ── Step 1: direct ID lookup (generator-provided) ────────────────────────
    uid = sub.get("usdm_entity_id")
    if not _is_null_id(uid):
        node = by_id.get(str(uid).strip())
        if node:
            nid = node.get("id")
            ntype = node.get("instanceType")
            return format_node_provenance(node), "id_lookup", str(nid) if nid else None, str(ntype) if ntype else None
        return f"(id `{uid}` not in protocol file)", "id_missing", None, None

    # ── Step 2: evaluator-driven broad search across ALL nodes ───────────────
    broad_hits = broad_usdm_search(pipd_text, by_id, top_k=5, min_ratio=min_lex)
    if broad_hits:
        score, node = broad_hits[0]
        line = format_node_provenance(node)
        nid   = node.get("id")
        ntype = node.get("instanceType")
        if use_ai and score < ai_below:
            utype_s = str(sub.get("usdm_entity") or "").strip()
            if utype_s:
                ai_line = try_ai_usdm_source(pipd_text, utype_s, type_index)
                if ai_line:
                    return ai_line, "ai_assisted", str(nid) if nid else None, str(ntype) if ntype else None
        label = "broad_eval"
        return f"{line} _(lex {score:.2f})_", label, str(nid) if nid else None, str(ntype) if ntype else None

    # ── Step 3: type-restricted fallback (generator's class) ─────────────────
    ...
    if not lex:
        return f"(no `{utype_s}` nodes in protocol)", "none", None, None
    ...
    suffix = f"_(lex {score:.2f})_" if score >= min_lex else f"_(weak lex {score:.2f})_"
    return f"{line} {suffix}", "type_match", str(nid) if nid else None, str(ntype) if ntype else None
