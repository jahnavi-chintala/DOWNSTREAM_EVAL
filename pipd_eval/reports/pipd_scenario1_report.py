...
replaces the default bullet synthesis with an LLM narrative; typed failures stay in the eval JSON
(``classified_failures``), not in the Word table.
...
from reports.pipd_composite_report import _PKG_DIR
from reports.pipd_eval_charts import try_write_eval_charts, write_score_per_category_chart
from reports.pipd_eval_report_reference import build_reference_eval_markdown, load_generator_report_meta
from reports.pipd_openai_report_enrichment import append_usdm_gt_sourced_section, fill_gsop_codes_with_openai
from reports.pipd_markdown_to_docx import write_docx_from_markdown
from core.pipd_structure_completeness import check_pipd_json
from reports.pipd_scenario1_report_yaml import write_scenario1_report_yaml
from utils.pipd_eval_config import category_weights_and_names, load_pipd_eval_config, resolve_eval_config_path
...
def _synthetic_per_category_from_s1(s1: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Index near-misses by category so they appear in Section 3 rows
    nm_by_cat: Dict[int, List[Dict[str, Any]]] = {}
    for nm in s1.get("near_misses") or []:
        try:
            c = int(nm.get("category_num") or 0)
        except (TypeError, ValueError):
            continue
        nm_by_cat.setdefault(c, []).append(nm)
    ...
    for key, block in raw.items():
        ...
        # Include near-miss items so they appear in the category detail table (Section 3)
        for nm in nm_by_cat.get(cn, []):
            rows.append(
                {
                    "ground_truth": str(nm.get("gt_text") or ""),
                    "generated": str(nm.get("generated_text") or ""),
                    "present": True,
                    "exact": False,
                    "semantic_f1": None,
                    "hallucination": False,
...
def write_scenario1_report_files(
    ...
    if (openai_reference_layout or layout_env) and os.getenv("OPENAI_API_KEY", "").strip():
        from reports.pipd_openai_reference_layout import rewrite_eval_markdown_like_reference
    ...
    if with_openai and os.getenv("OPENAI_API_KEY", "").strip():
        from reports.pipd_composite_report import add_ai_narrative_section
    ...
    # Apply Scenario 1 improvement overlay (weighted overall score + summary +
    # structured §4 Improvement Actions + Out-of-scope section). Runs last so
    # it wins over the reference-layout rewrite.
    try:
        from scripts.pipd_scenario1_improvement import apply_to_markdown as _apply_s1_improvement

        md, _s1_imp_payload = _apply_s1_improvement(md, s1, result)
        if _s1_imp_payload:
            result["scenario1_improvement"] = _s1_imp_payload
    except Exception as exc:
        result["scenario1_improvement_error"] = str(exc)
