...
from core.eval_scenario1 import NUM_CATEGORIES, classify_failures as pipd_classify_failures
from utils.miss_explanation import write_miss_explanation as pipd_write_miss_explanation
...
_DEFAULT_GT_CANDIDATES = (
    _PKG_ROOT / "data" / "pipd_ground_truth_clean.csv",
    _PKG_ROOT / "data" / "pipd_ground_truth.csv",
    _PKG_ROOT.parent / "pipd_ground_truth.csv",
)
_DEFAULT_DEV_CANDIDATES = (
    _PKG_ROOT / "data" / "deviation_subcategories_clean.csv",
    _PKG_ROOT / "data" / "deviation_subcategories.csv",
    _PKG_ROOT.parent / "deviation_subcategories.csv",
)
...
def _pipd_preview(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if sc == 1:
        ...
        overall_pct = result.get("overall_score_percent")
        prev["headline"] = {
            "verdict": result.get("go_no_go"),
        ...
        rows.append(
            {
                "metric": "Overall score",
                "detail": (
                    f"{overall_pct:.1f}% — {rag or '—'} "
                    f"({health.get('pass', 0)} PASS · {health.get('warn', 0)} WARN · {health.get('fail', 0)} FAIL)"
                    if overall_pct is not None
                    else f"{rag or '—'}"
                ),
                "pass": str(rag or "").upper() in ("GREEN", "AMBER"),
                "tooltip": (
                    "Weighted signal health across S1-S7 (PASS=1.0 · WARN=0.5 · FAIL=0.0). "
                    "GO requires verdict GREEN or AMBER — any FAIL flips the doc to NO-GO."
                ),
                "hero": True,
            }
        )
        _signal_labels = {
            "S1_HALLUCINATION_CHECK":        ("S1 Hallucination check",
                "Every auto_confirmed subcategory must have a non-null usdm_entity + benchmark rate > 0."),
            "S2_CONFIDENCE_DISTRIBUTION":    ("S2 Confidence distribution",
                "Within each category, >50% low_confidence triggers WARN (generator struggling)."),
            "S3_CATEGORY_COMPLETENESS":      ("S3 Category completeness",
                "All 11 PIPD categories must be present in the generator output."),
            "S4_PROTOCOL_SPECIFICITY_CAT10": ("S4 Cat 10 specificity",
                "Category 10 (Other) subcategories must never be auto_confirmed — protocol-specific always."),
            "S5_BENCHMARK_ALIGNMENT":        ("S5 Benchmark alignment",
                "Auto_confirmed rate ≥ 0.70 (segment rate from the deviation benchmarks CSV)."),
            "S6_SUBCAT_COUNT_SANITY":        ("S6 Subcategory count sanity",
                "Generated count per category within 3× of historical average."),
            "S7_NONE_IDENTIFIED_PLAUSIBILITY": ("S7 None-identified plausibility",
                "none_identified=true only where the benchmark supports a low historical rate."),
        }
        for sid, (label, tip) in _signal_labels.items():
        ...
        prev["counts"] = {
            "human_review_items": int(result.get("human_review_count") or 0),
...
def register_eval_upload_routes(app, *, route_prefix: str = "") -> None:
    ...
    async def eval_upload_session(
        ...
        try:
            paths = pipd_write_miss_explanation(
                result,
                str(usdm_path.resolve()),
                out_dir,
                f"pipd_eval_{study_id}",
            )
            miss_json_p = paths["json"]
            miss_md_p = paths["md"]
        except Exception as exc:  # pragma: no cover - defensive
            print(f"[WARN] PIPD miss explanation skipped: {exc}")
