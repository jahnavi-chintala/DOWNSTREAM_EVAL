...
structural parity with ``reference_spec/pipd_eval_B7981027.json``.
"""
...
def align_to_reference(ref: Any, data: Any) -> Any:
    ...
    if isinstance(ref, dict):
        ...
        for k, rv in ref.items():
            # Try exact match first, then int<->str coercion for JSON interop
            if k in src:
                sv = src[k]
                ...
                # Key not present in live data — fill from reference shape only
