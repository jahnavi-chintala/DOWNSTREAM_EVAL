"""
PIPD Scenario 1 — Miss Explanation Generator.

For every GT subcategory line in ``per_category[*].missed_subcats`` that the
generator failed to emit, classify WHY the miss happened using the Field
Provenance Policy (`miss_policy.yaml`) plus a live index of the uploaded USDM
JSON.
...
# PIPD has 6 numeric categories keyed under `per_category`. The policy
# addresses them by well-known *names*; map category num → policy key.
_PIPD_CATEGORY_NAME = {
    1: "eligibility",
    2: "population",
    3: "endpoints",
...
def _policy_for_category(cat_num: int, policy: Dict[str, Any]
                         ) -> Tuple[str, Optional[str], List[str], str]:
    """Return (policy_class, required_type, anchors, section_name)."""
    cats = (policy or {}).get("categories") or {}
    default_class = (policy or {}).get("default_class") or _CLASS_INFERRED
    default_anchors = list((policy or {}).get("default_anchors") or [])
    section = _PIPD_CATEGORY_NAME.get(cat_num, f"category_{cat_num}")
    entry = cats.get(section) or {}
    cls = str(entry.get("class") or default_class).upper()
    req = entry.get("required_type")
    anchors = list(entry.get("anchors") or [])
    if not anchors:
        anchors = default_anchors
    return cls, req, anchors, section


def _classify(gt_text: str,
              cat_num: int,
              idx: Optional[UsdmIndex],
              policy: Dict[str, Any]) -> Dict[str, Any]:
    cls, required_type, anchors, section = _policy_for_category(cat_num, policy)
    base = {
        "section": section,
...
def _iter_pipd_misses(report: Dict[str, Any]) -> List[Dict[str, Any]]:
    out: List[Dict[str, Any]] = []
    per_cat = report.get("per_category") or {}
    for cat_num, cblock in per_cat.items():
        try:
            cn = int(cat_num)
        except Exception:
            continue
        for text in (cblock or {}).get("missed_subcats") or []:
            out.append({"category_num": cn, "gt_text": str(text)})
    return out
...
def _render_markdown(payload: Dict[str, Any]) -> str:
    ...
    lines: List[str] = []
    lines.append(f"# PIPD Miss Explanation — {payload['study_id']}")
    lines.append("")
    ...
    lines.append("")
    lines.append("| # | Category | GT Text | Policy | Verdict | Reason |")
    lines.append("|---|---|---|---|---|---|")
    for i, it in enumerate(items, 1):
        ...
        lines.append(
            f"| {i} | `{it['section']}` (cat {it['category_num']}) | {gt_text} | "
...
_DEFAULT_POLICY = _PKG / "miss_policy.yaml"
...
def build_miss_explanation(
    ...
    *,
    ...
    misses = _iter_pipd_misses(report)
    items = [_classify(m["gt_text"], m["category_num"], idx, policy) for m in misses]
